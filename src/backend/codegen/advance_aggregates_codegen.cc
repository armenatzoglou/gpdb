...
bool AdvanceAggregatesCodegen::GenerateAdvanceTransitionFunction(
  ...
  // If transition function is strict then check if there are null arguments
  // or if transition value is null, or if transvalue has been set
  if (peraggstate->transfn.fn_strict) {
    ...
    llvm::BasicBlock* strict_check_noTransValue_block = codegen_utils->
        CreateBasicBlock("strict_check_noTransValue_block",
                         pg_func_info->llvm_main_func);
    // Block that contains instructions that will be executed when transfn
    ...
    // --------------------
    // Checks if there is a NULL argument. If yes then goes to
    // null_argument_block; generate_function_block otherwise.
    irb->SetInsertPoint(strict_check_for_null_args_block);
    ...
    for (int i = 1; i < pg_func_info->llvm_args_isNull.size(); ++i) {
      irb->CreateStore(
    ...
    irb->CreateCondBr(irb->CreateLoad(llvm_there_is_null_arg_ptr),
                      ...
                      strict_check_noTransValue_block /*false*/);
    ...
    llvm::Value* llvm_arg1_ptr = irb->CreateAlloca(
        ...
        nullptr, "llvm_arg1_ptr");

    if (pg_func_info->llvm_args.size() > 1) {
      irb->CreateStore(pg_func_info->llvm_args[1],
                       llvm_arg1_ptr);
    } else {
    ...
    newVal = irb->CreateCall(llvm_datumCopyWithMemManager,
  ...
  // transValueIsNull is passed as argument to PGFuncGeneratorInfo
